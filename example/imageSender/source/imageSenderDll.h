...
struct ImageSessionFaultInjectionSettings {
    FaultImageFunc faultFunction;
    void* faultFunctionContext;
};

enum ImageSenderSessionProperty {
    FaultInjectionSettings = 1,
};

struct ImageSessionDebugInfo {
    uint64_t T0World;
    uint64_t TRxRender;
};
...
#define imageSender_Error_InvalidProperty   -10005
...
int32_t _IMAGE_SENDER_FUNC CancelImageSenderSession(ImageSenderSessionHandle sessionHandle);
...
int32_t _IMAGE_SENDER_FUNC SetImageSenderSessionProperty(ImageSenderSessionHandle sessionHandle, ImageSenderSessionProperty property, const void* value);

int32_t _IMAGE_SENDER_FUNC SendImageBlockWithDebugInfo(ImageSenderSessionHandle sessionHandle, 
                                                       const ImageBlock* imageBlock, 
                                                       const ImageSessionDebugInfo* debugInfo, 
                                                       int32_t timeoutMs);
